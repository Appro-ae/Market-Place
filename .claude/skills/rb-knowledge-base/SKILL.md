---
name: rb-knowledge-base
description: >-
  Reem Bank (RF) platform knowledge base for agent KT — Super Portal modules,
  queue model and drop points, credit decisioning and the DBR safety nets,
  pre-dedupe/30-day rules, permissions, audit-trail and email conventions
  (Client/Bank), the cancellation pattern, the Application Revert feature, the
  portal design system, the appro BRD house style and the screenshot/composite
  toolchain. Load when working on any Reem Bank / RF requirement, ticket, BRD,
  user story or screen so the platform context is in hand.
---
...
**Bundled screens:** when this skill is uploaded to a workspace, the key
visuals ship with it under `screens/` — SC1–SC6 + `Flow_Application_Revert.png`
(the Revert feature screens, composited over real UAT captures) and the
`ia_*.png` impact-analysis reference crops (Role Management, queue menu,
email-template list, Report Enquiry, Strategies/Versions menu, application
history, status-transition and services diagrams). In the Market-Place repo
the originals live in `docs/brd/` and `docs/brd/assets/`.
