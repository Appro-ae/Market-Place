...
| 06/09 | Round 5 (apply + approval) | Round-5 edits (27 ops) hit `CONTENT_LIMIT_EXCEEDED` at 92 KB: shrunk with `shrink_adf.py --strike-only`, then rebuilt with `--strip-old-strikes` (round-4 removals accepted, round-5 removals still struck) → 75 KB accepted. Live check: 12 tables, 59 rows, 104 bullets, 8 cards, 107 purple runs, 29 strikes; ADF text = live text apart from renderer artifacts. | Approval comment posted on the user's "post": R1–R7 Done (R7 flagged), two tickets to raise (Distribution Portal `staff_id` dependency, notification audit), mockup updates; the PO moved the ticket to Ready for Dev herself minutes later. |
| 07/09 | Terminology (round 6) | The PO asked which term ADIB uses in its UTM links. Verified: parameter `saleagentCode` in the spec tables (ADIB-106, ADIB-2640, ADIB-6725), `salesagentCode` in the example URLs, `saleAgentCode` in CVC production (ADIB-5268); the mismatch is an open defect (ADIB-6722) left unfixed because production links would break. UI field "Staff Referral Code", API field `referralCode`. | On "change the US whereby Staff ID = Sale agent code": three tracked edits on a fresh snapshot (Context definition, AC1 label, IA4), parameter key `staff_id` kept; 76 KB after `--strike-only`. Lesson: answer the terminology question with the exact spellings per ticket, then let the PO decide the scope of the rename. |
| 07/09 | Rename (round 7) | "Change all Staff ID in US to Sale Agent Code (Sales Code / Agent Code / Employee ID)". The PO had already renamed six labels by hand in purple bold; the fresh snapshot showed it. | Kept her edits; `replace_between` over the whole document for the remaining 83 + 3 plural occurrences and the parameter `staff_id` → `sale_agent_code`; full form on labels and the definition, short form in rules; one purple terminology paragraph instead of 90 tracked pairs (size and readability); the specific ops ran before the global ones and the note after, so the note's own "Staff ID" survived. 77 KB. |
