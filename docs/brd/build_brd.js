...
P(
  ...
  p('Scope rule: only rejections with a **Credit root cause** are revertible — **rejected by Credit** in Credit Queue, or **auto-rejected by the System for credit reasons** (DBR safety nets, segmentation failure, approved limit below Min Boundary). Compliance, Risk and Sale queue rejections are **out of scope**.'),
  tbl(
...
P(
  ...
  bullet("Audit trail is captured with: Step = **'Manual revert process'**, State = 'Rejected', Step Status = 'Successful', Step Details = **'Revert requested by %Username% to Credit Queue. Revert reason is <REVERT_REASON captured above>'**, Action by = <user email id>."),
...
P(
  ...
  bullet('**Customer communication — final confirmation only:** the customer already received the rejection notification at the point of decision and is **not** notified of the reopen. The customer is informed only when the re-assessment completes: on approval, the customer receives the approval communication below — the same message as a normally approved application; on a repeat rejection, the standard rejection notification applies and the 30-day window restarts.'),
  spacer(60),
  tbl(
    ['Type', 'Subject', 'Content', 'Trigger'],
    [['Email (Customer)', 'Reem Bank — Update on your application %%APPLICATION_ID%%', 'Dear %%CUSTOMER_NAME%%, Following a further review of your %%PRODUCT_TYPE%% application %%APPLICATION_ID%%, we are pleased to inform you that your application has been approved. No action is required from your side — we will be in touch with the next steps to complete your application. Thank you for choosing Reem Bank. Best Regards, Reem Bank', 'One time — when an application reopened via an approved revert is approved after re-assessment. New Customer-type template in Communication Setup, mirroring the standard approval message.']],
    [15, 22, 40, 23],
  ),
  h2('7. Summary'),
  tbl(
    ...
    [
      ...
      ['4', 'Queue User (Credit)', 'Works the case as normal — edits credit parameters, then approves, overrides or rejects', 'Awaiting Credit Approval', 'Application re-decisioned against the currently published credit policy; on approval the customer receives the approval communication (section 6), on rejection the 30-day window restarts'],
    ],
    [8, 17, 31, 20, 24],
  ),
);
